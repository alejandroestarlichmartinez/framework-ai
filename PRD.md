...
```mermaid
flowchart TD
    START([framework-ai install]) --> DETECT
    ...
    subgraph PHASE_3["Phase 3: Backup"]
        BACKUP[Backup existing configs<br/>~/.framework-ai-backup-TIMESTAMP/]
    end
...
| Homebrew | `brew install Gentleman-Programming/tap/framework-ai` | P0 |
| Go install | `go install github.com/alejandroestarlichmartinez/framework-ai/cmd/framework-ai@latest` | P1 |
| Direct binary | Download from GitHub Releases | P1 |
| winget (Windows) | `winget install framework-ai` | P2 |
...
**Requirements:**
- R-UPDATE-01: The installer MUST support `framework-ai update` to check for and install newer versions of itself
- R-UPDATE-02: The installer MUST support `framework-ai update --skills` to pull latest skill versions for all configured agents
- R-UPDATE-03: The installer MUST support `framework-ai update --engram` to update Engram to the latest version
...
- R-REL-03: The installer MUST support `framework-ai repair` to re-run failed steps
...
1. **Naming**: `framework-ai`, `framework-ai`, `gai`, or something else? Should it be part of the `Gentleman-Programming` org or standalone?
...
framework-ai install --preset gentleman --agents claude-code,opencode

# Minimal setup, just Claude Code with basic security
framework-ai install --preset minimal --agents claude-code

# Team provisioning from shared profile
framework-ai install --profile ./team-ai-config.yaml

# Update all skills to latest
framework-ai update --skills

# Update Engram
framework-ai update --engram

# Backup current configs
framework-ai backup

# Restore from backup
framework-ai restore --list
framework-ai restore --id 2026-02-27-143022
