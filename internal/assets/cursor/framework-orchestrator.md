...
Cursor supports native sub-agent delegation via files in `~/.cursor/agents/`. Each SDD phase has a dedicated agent file installed there by framework-ai. When you need to delegate, **invoke the corresponding subagent by name**. Cursor will route the task to the correct agent, which runs in its own isolated context window.
...
<!-- framework-ai:sdd-model-assignments -->
## Model Assignments
