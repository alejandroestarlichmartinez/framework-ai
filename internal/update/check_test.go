...
func TestDetectInstalledVersion(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "framework-ai uses build var",
			tool:         ToolInfo{Name: "framework-ai", DetectCmd: nil},
			currentBuild: "1.5.0",
			wantVersion:  "1.5.0",
		},
		{
			name:         "framework-ai dev build",
			tool:         ToolInfo{Name: "framework-ai", DetectCmd: nil},
			currentBuild: "dev",
			wantVersion:  "dev",
		},
...
func TestFetchLatestRelease_GithubToken(t *testing.T) {
	...
	if gotUserAgent != "framework-ai-update-check" {
		t.Fatalf("User-Agent = %q, want %q", gotUserAgent, "framework-ai-update-check")
	}
}
...
func TestCheckAll(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case contains(path, "framework-ai"):
			release = githubRelease{TagName: "v1.5.0", HTMLURL: "https://github.com/alejandroestarlichmartinez/framework-ai/releases/tag/v1.5.0"}
	...
	// framework-ai: 1.5.0 local == 1.5.0 remote → UpToDate
	assertResult(t, results[0], "framework-ai", UpToDate, "1.5.0", "1.5.0")
...
func TestCheckAll_NetworkError(t *testing.T) {
	...
	// framework-ai has no DetectCmd, so it gets currentBuildVersion "1.0.0" as local
	// but fetch fails → CheckFailed (it has a local version).
	if results[0].Status != CheckFailed {
		t.Fatalf("framework-ai status = %q, want %q", results[0].Status, CheckFailed)
	}
	if results[0].Err == nil {
		t.Fatalf("framework-ai expected error, got nil")
	}
...
func TestUpdateHint(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "framework-ai macOS",
			tool:    ToolInfo{Name: "framework-ai"},
			profile: system.PlatformProfile{OS: "darwin", PackageManager: "brew"},
			want:    "brew upgrade framework-ai",
		},
		{
			name:    "framework-ai linux",
			tool:    ToolInfo{Name: "framework-ai"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "curl -fsSL https://raw.githubusercontent.com/alejandroestarlichmartinez/framework-ai/main/scripts/install.sh | bash",
		},
		{
			name:    "framework-ai windows",
			tool:    ToolInfo{Name: "framework-ai"},
			profile: system.PlatformProfile{OS: "windows", PackageManager: "winget"},
			want:    "irm https://raw.githubusercontent.com/alejandroestarlichmartinez/framework-ai/main/scripts/install.ps1 | iex",
		},
		{
		...
		},
		{
			...
			tool:    ToolInfo{Name: "engram"},
			profile: system.PlatformProfile{OS: "linux", PackageManager: "apt"},
			want:    "framework-ai upgrade (downloads pre-built binary)",
		},
		{
			name:    "engram windows",
			tool:    ToolInfo{Name: "engram"},
...
func TestRegistryContents(t *testing.T) {
	...
	expected := map[string]struct {
		...
		"framework-ai":                    {owner: "Gentleman-Programming", repo: "framework-ai"},
	...
	// framework-ai must have nil DetectCmd.
	if Tools[0].DetectCmd != nil {
		t.Fatalf("framework-ai DetectCmd should be nil")
	}
...
func TestCheckAll_DevVersion(t *testing.T) {
	...
	// Override only the first tool (framework-ai) by running CheckAll with "dev".
	origTools := Tools
	t.Cleanup(func() {
		httpClient = origClient
		lookPath = origLookPath
	...
	// Restrict to just framework-ai to isolate the test.
	...
	if results[0].Status != DevBuild {
		t.Fatalf("framework-ai dev status = %q, want %q", results[0].Status, DevBuild)
	}
}
...
// and that the dev-build sentinel causes framework-ai to be reported as DevBuild.
...
// when the running framework-ai binary reports version "dev", it is identified as a
// DevBuild and NOT reported as UpdateAvailable or VersionUnknown.
//
// The spec says:
//   - Dev build MUST be reported as development-build semantic
//   - framework-ai self-upgrade is skipped while engram/gga remain eligible
func TestCheckFiltered_DevBuildSemanticsForGentleAI(t *testing.T) {
	...
	lookPath = func(string) (string, error) { return "", fmt.Errorf("not found") }
	execCommand = func(name string, args ...string) *exec.Cmd { return exec.Command("false") }
	Tools = []ToolInfo{Tools[0]} // framework-ai only
	...
	r := results[0]
	if r.Tool.Name != "framework-ai" {
		t.Fatalf("tool = %q, want framework-ai", r.Tool.Name)
	}
...
// framework-ai with "dev" version gets DevBuild while engram with a real version stays eligible.
func TestCheckFiltered_DevBuildSkipNotEligible(t *testing.T) {
	...
	// Only framework-ai and engram for this test
	...
	// framework-ai should be DevBuild
	if results[0].Status != DevBuild {
		t.Fatalf("framework-ai status = %q, want DevBuild", results[0].Status)
	}
...
func TestNoUpdatesPath(t *testing.T) {
	...
	// Only engram and gga for this test (skip framework-ai to avoid dev-build behavior)
