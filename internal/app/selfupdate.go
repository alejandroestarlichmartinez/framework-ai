...
// selfUpdate checks for and applies a framework-ai update before normal dispatch.
...
func selfUpdate(ctx context.Context, version string, profile system.PlatformProfile, stdout io.Writer) error {
	...
	// Check for updates (only framework-ai).
	results := updateCheckFiltered(ctx, version, profile, []string{"framework-ai"})

	// Find the framework-ai result.
	var target *update.UpdateResult
	for i := range results {
		if results[i].Tool.Name == "framework-ai" {
			target = &results[i]
			break
		}
	}
	...
	for _, r := range report.Results {
		if r.ToolName == "framework-ai" && r.Status == upgrade.UpgradeSucceeded {
			succeeded = true
			break
		}
	}
	...
	//
	// Use exec.LookPath("framework-ai") rather than os.Executable() because
	// on Homebrew, os.Executable() resolves to the versioned Cellar path
	// (e.g. /opt/homebrew/Cellar/framework-ai/1.8.5/bin/framework-ai) which
	// still points to the OLD binary after upgrade. The PATH symlink
	// (/opt/homebrew/bin/framework-ai) is updated by Homebrew to the new
	// version, so LookPath gives us the correct binary.
	executable, err := lookPathFn("framework-ai")
	if err != nil {
