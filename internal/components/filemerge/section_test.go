...
const gentleAiMarkerSection = `<!-- framework-ai:persona -->
## Personality
...
func TestStripLegacyPersonaBlock_FingerprintInsideMarkerReturnsSame(t *testing.T) {
	// Fingerprints only exist inside framework-ai markers — should NOT be stripped.
...
func TestStripLegacyPersonaBlock_LegacyBlockBeforeMarkersStripped(t *testing.T) {
	...
	if !strings.Contains(result, "<!-- framework-ai:persona -->") {
		t.Fatal("stripped result missing framework-ai marker section")
	}
}
...
func TestStripLegacyPersonaBlock_MixedZone_OnlyOneFingerprint_PreMarker(t *testing.T) {
	...
	// exist only inside a framework-ai marker block.
	//
	...
	markerWithOtherFingerprints := "<!-- framework-ai:persona -->\n## Personality\n\nSenior Architect, 15+ years experience.\n<!-- /framework-ai:persona -->\n"
...
func TestStripLegacyPersonaBlock_MixedZone_TwoFingerprints_PreMarker(t *testing.T) {
	...
	markerWithRule := "<!-- framework-ai:persona -->\n## Rules\n\n- Rule inside marker.\n<!-- /framework-ai:persona -->\n"
...
func TestStripLegacyPersonaBlock_AllFingerprintsPreMarker_Strips(t *testing.T) {
	...
	markerSection := "<!-- framework-ai:persona -->\nUpdated persona.\n<!-- /framework-ai:persona -->\n"
...
func TestStripLegacyPersonaBlock_UserContentBeforeAndAfterMarkersPreserved(t *testing.T) {
	...
	if !strings.Contains(result, "# Custom section") {
		t.Fatal("content after framework-ai markers must be preserved")
	}
}
...
func TestStripLegacyATLBlock_ATLBlockThenMarkers_StripsATLKeepsMarkers(t *testing.T) {
	...
	if !strings.Contains(result, "<!-- framework-ai:framework-orchestrator -->") {
		t.Fatal("framework-orchestrator marker section must be preserved")
	}
	if !strings.Contains(result, "<!-- /framework-ai:framework-orchestrator -->") {
		t.Fatal("framework-orchestrator close marker must be preserved")
	}
}
...
func TestStripLegacyATLBlock_ContentBeforeATL_StripsOnlyATL(t *testing.T) {
	...
	if !strings.Contains(result, "<!-- framework-ai:framework-orchestrator -->") {
		t.Fatal("framework-orchestrator section must be preserved")
	}
}
...
func TestStripLegacyATLBlock_ATLBlockAndSDDOrchestrator_StripsOnlyATL(t *testing.T) {
	...
	if !strings.Contains(result, "<!-- framework-ai:framework-orchestrator -->") {
		t.Fatal("framework-orchestrator section must be preserved after ATL strip")
	}
	if !strings.Contains(result, "You are a COORDINATOR.") {
		t.Fatal("framework-orchestrator content must be preserved")
	}
}
