...
func inject(configHomeDir, promptDir string, adapter agents.Adapter) (InjectionResult, error) {
	...
	case model.StrategySeparateMCPFiles:
		...
		// `engram setup <agent>` is invoked. framework-ai's Inject() runs after
		...
		// See: https://github.com/alejandroestarlichmartinez/framework-ai/issues (engram absolute path regression)
...
// framework-ai runs Inject() after setup, so we must not overwrite that absolute
