...
// Example: <!-- framework-ai:custom-agent:my-skill -->
const markerFormat = "<!-- framework-ai:custom-agent:%s -->"
...
func buildSDDBlock(agent *GeneratedAgent, marker string) string {
	...
	endMarker := fmt.Sprintf("<!-- /framework-ai:custom-agent:%s -->", agent.Name)
	return marker + "\n" + body + endMarker
}
...
func replaceBlock(content, marker, newBlock string) string {
	endMarker := fmt.Sprintf("<!-- /framework-ai:custom-agent:%s -->", extractName(marker))
...
// Example: "<!-- framework-ai:custom-agent:my-skill -->" → "my-skill"
func extractName(marker string) string {
	prefix := "<!-- framework-ai:custom-agent:"
	suffix := " -->"
