...
- WHEN `framework-ai sync --profile cheap:anthropic/claude-haiku-3.5-20241022` runs
...
- GIVEN `framework-ai sync --profile badformat` is run (no colon separator)
...
- GIVEN `framework-ai sync --profile cheap:haiku --profile-phase cheap:sdd-apply:sonnet`
- WHEN sync runs
