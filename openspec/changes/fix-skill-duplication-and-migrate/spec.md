...
- After `framework-ai install` or `framework-ai sync` on any `~/.claude/` directory, none of the 11 SDD SKILL.md entries MUST appear as user-invocable items in the Claude Code `/` picker.
...
- GIVEN a fresh `~/.claude/skills/` directory with no prior framework-ai install
- WHEN `framework-ai install` runs with the Claude adapter selected
...
- GIVEN a Claude Code v2.1.131+ session with the new SKILL.md files installed via `framework-ai sync`
...
- After `framework-ai install`, each refactored skill directory on disk MUST contain both `SKILL.md` and its `references/` subdirectory.
...
- WHEN `framework-ai install` runs for the Claude adapter
...
- WHEN `framework-ai install` runs
...
- Migration logic — existing installs pick up new content automatically on next `framework-ai sync`.
