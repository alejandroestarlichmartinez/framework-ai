...
```
cmd/framework-ai/             CLI entrypoint
internal/
...
framework-ai install --dry-run --agent claude-code --preset minimal

# Dry-run smoke test (Windows PowerShell)
framework-ai.exe install --dry-run --agent claude-code --preset minimal
```
