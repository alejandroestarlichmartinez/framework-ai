...
This document explains how framework-ai integrates with **Kiro IDE** and what is installed in your local Kiro configuration.

## Overview

framework-ai supports Kiro as a **native-subagent** platform (`kiro-ide`).

When configured, framework-ai installs:

| Artifact | Path |
|----------|------|
| Steering file | `~/.kiro/steering/framework-ai.md` |
...
> **Auto-install not supported.** Kiro must be installed manually before running framework-ai.
...
framework-ai uses **two signals** to detect Kiro:
...
In practice: **the installer detects Kiro from `~/.kiro`**, not from `PATH`. If you have Kiro installed but `~/.kiro` hasn't been created yet (e.g., before first launch), run Kiro once to initialize its config dir, then re-run `framework-ai install`.
...
This follows the same SDD architecture used in framework-ai: orchestrator coordinates, phase agents execute, Engram persists artifacts across phases.
...
Kiro has a built-in spec workflow that framework-ai leverages. For medium and large changes, the orchestrator will use native Kiro artifacts at:
...
The steering file written by framework-ai uses the following frontmatter:
...
| Steering file | `%USERPROFILE%\.kiro\steering\framework-ai.md` |
...
Kiro uses a **split-root layout** — framework-ai managed files and IDE settings live in different directories:

- **Steering, skills, and native agents** → `~/.kiro/` (or `%USERPROFILE%\.kiro\` on Windows)
  - `~/.kiro/steering/framework-ai.md` — orchestrator persona
...
If framework-ai skills or steering are missing, check `~/.kiro/skills/` and `~/.kiro/steering/`.
