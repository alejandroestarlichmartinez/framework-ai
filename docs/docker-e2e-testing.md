...
End-to-end tests that validate the `framework-ai` installer binary inside Docker containers running real Linux distributions.
...
docker build -f e2e/Dockerfile.ubuntu -t framework-ai-e2e-ubuntu .
docker run --rm framework-ai-e2e-ubuntu

# Run with full E2E on Arch
docker build -f e2e/Dockerfile.arch -t framework-ai-e2e-arch .
docker run --rm -e RUN_FULL_E2E=1 framework-ai-e2e-arch

# Interactive debugging
docker run --rm -it framework-ai-e2e-ubuntu /bin/bash
```
...
4. Build the `framework-ai` binary for `linux/amd64`
