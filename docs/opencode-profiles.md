...
1. **Generated multi-profile mode** -- the classic Gentle AI flow. The base SDD conductor is `framework-orchestrator`. Each named profile generates its own `sdd-orchestrator-{name}` plus 10 suffixed sub-agents in `opencode.json`, and you switch between them with **Tab**.
...
1. Launch the installer: `framework-ai` (or `go run ./cmd/framework-ai`).
...
Open OpenCode and press **Tab** -- your new orchestrator appears alongside `framework-orchestrator`, the default OpenCode SDD conductor.
...
The effort options are populated from a cache file written by the bundled `model-variants` OpenCode plugin at `~/.framework-ai/cache/model-variants.json`. The plugin runs the first time OpenCode starts after `framework-ai sync` and refreshes the cache on every subsequent start.

**First-run order matters:**

1. Run `framework-ai` (installs the plugin into `~/.config/opencode/plugins/`).
2. Run `opencode` once -- on startup the plugin queries the provider list and writes `~/.framework-ai/cache/model-variants.json`.
3. Re-run `framework-ai` and open the model picker. Reasoning models now show the effort selector.
...
|---|---|---|
| `framework-orchestrator` | Canonical base OpenCode SDD conductor. All `/sdd-*` commands point here by default. | No |
| `sdd-orchestrator` | Legacy base conductor key. Sync migrates it to `framework-orchestrator`. | No; let sync migrate it |
...
```bash
framework-ai sync --profile cheap:anthropic/claude-haiku-3.5-20241022
```
...
On `framework-ai sync`, if OpenCode profile files exist under:
...
```bash
framework-ai sync --agent opencode --sdd-profile-strategy generated-multi
```
...
- **preserves the current `framework-orchestrator` prompt** during sync so external tools can keep their runtime policy / fallback blocks intact
...
|---|---|
| `framework-orchestrator` | Default profile (your original config) |
...
If you're using an external single-active manager instead, you typically keep working with the base `framework-orchestrator` while the external tool swaps its active model assignments at runtime.
...
The `default` profile (`framework-orchestrator`) can be edited but not deleted -- it always exists when SDD is configured.
...
- **`generated-multi`** -- scan `opencode.json` for `sdd-orchestrator-*`, update shared prompts, regenerate profile orchestrators, preserve model assignments, and keep `framework-orchestrator` as the canonical base conductor
- **`external-single-active`** -- detect external profile files, keep the shared SDD assets current, and preserve the existing `framework-orchestrator` prompt instead of overwriting external runtime extensions
