...
When `framework-ai` installs GGA on Windows, it now installs a `gga.ps1` wrapper
...
check — re-running `framework-ai install` is idempotent.
...
`framework-ai install`**. In the final design this was moved to **runtime** — the `.ps1`
