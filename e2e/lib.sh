#!/usr/bin/env bash
# lib.sh — shared test helpers for framework-ai E2E tests
...
# Binary resolution
# ---------------------------------------------------------------------------
# The binary should be built and placed at /usr/local/bin/framework-ai inside
# the Docker container. If not found, fall back to $HOME/framework-ai or the
# current directory.
# Resolution priority (highest → lowest):
#   1. ./framework-ai in the current repo directory (freshly built local binary)
#   2. ~/framework-ai (explicit copy in home)
#   3. framework-ai on PATH (system-installed, e.g. Homebrew)
# This ensures `go build ./cmd/framework-ai && bash e2e/e2e_test.sh` always
# tests the locally built binary rather than the installed release version.
resolve_binary() {
    # Prefer the locally built binary (./framework-ai) produced by `go build ./cmd/framework-ai`.
    ...
    if [ -x "$repo_root/framework-ai" ]; then
        echo "$repo_root/framework-ai"
    elif [ -x "./framework-ai" ]; then
        echo "./framework-ai"
    elif [ -x "$HOME/framework-ai" ]; then
        echo "$HOME/framework-ai"
    elif command -v framework-ai >/dev/null 2>&1; then
        echo "framework-ai"
    else
        echo ""
    fi
...
cleanup_test_env() {
    ...
    rm -rf "$HOME/.framework-ai" 2>/dev/null || true
...
#
# Full Docker E2E validates framework-ai's agent/config injection behavior, not the
...
setup_fake_engram_binary() {
    ...
    local fake_bin_dir="$HOME/.framework-ai-e2e/bin"
...
# Checks that the framework-ai section marker appears exactly once (no duplicates).
assert_no_duplicate_section() {
    ...
    local marker="<!-- framework-ai:${section_id} -->"
    local count
